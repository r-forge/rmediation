qprodnormal <-
function(p, mu.x, mu.y, se.x, se.y, rho=0, lower.tail=TRUE, root.eps=1e-8,...){
    ...
    if (lower.tail==FALSE) {
        u0<- mu.a*mu.b +6*sqrt(1+mu.a^2+mu.b^2) #upper
        l0=mu.a*mu.b
        alpha<- 1- p
    }
    else{
        l0<-mu.a*mu.b - 6*sqrt(1+mu.a^2+mu.b^2)#lower
        u0<-mu.a*mu.b
        alpha<- p
    }
    gx=function(x, z) {
    ...
    while (p.l>0) {
        ...
        l0=l0-0.5
        p.l<-integrate(gx,lower=-Inf,upper=Inf, z=l0)$value-alpha
        if (iter> 100 ) stop(" No initial valid lower bound interval!")
    }
    ...
    while (p.u<0) {
        ...
        u0=u0+0.5
        p.u<-integrate(gx,lower=-Inf,upper=Inf, z=u0)$value-alpha
        if (iter> 100 ) stop(" No initial valid upper bound interval!")
    }
    ...
    while(eps > root.eps) {
        ...
        p.l<-integrate(gx,lower=-Inf,upper=Inf, z=l0)$value-alpha
        p.u<-integrate(gx,lower=-Inf,upper=Inf, z=u0)$value-alpha
        p.new<-integrate(gx,lower=-Inf,upper=Inf, z=new)$value-alpha
        iter<-iter+1
    ...
    new <- new*se.x*se.y
    ##cat('quantile= ',new,'\n')
    return(new)                          #returns quantile corresponding to alpha
